...
def process_data_with_formatter(corpus_data):
    ...
    for corpus_name in ["Brent", "NewEngland", "Post", "Bloom", "Brown", "HSLLD", "Kuczaj", "Sachs", "VanKleeck"]:
        if corpus_name not in corpus_data["Corpora_modified"]:
            continue

        result["Corpora_modified"][corpus_name] = {}
        corpus_data_current = corpus_data["Corpora_modified"][corpus_name]

        for dir_name, content in corpus_data_current.items():
            if "files" not in content:
                continue

            result["Corpora_modified"][corpus_name][dir_name] = {"files": []}
            ...
            for file in content["files"]:
                ...
                result["Corpora_modified"][corpus_name][dir_name]["files"].append(
                    processed_file
                )
